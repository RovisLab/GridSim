...
def get_elements_from_gridsim_record_file(tmp_fp):
	with open(tmp_fp, "r") as f:
        ...
        for line in lines:
        	try:
        	    f_elems = [float(x) for x in line.split(",")]
        	except ValueError:
        		pass
...
class FrontSensorArrayTrainingSet(object):
    ...
    def write_output_data(history_data, prediction_data, actions_data, prev_actions_data, val):
        ...
        with open(obs_file, "a") as f:
        	for h in history_data:
        		...
        		for h_elem in h:
        			f.write("{0},".format(h_elem))
        		f.write("\n")

        with open(pred_file, "a") as f:
        	for p in prediction_data:
        		for p_elem in p:
        			f.write("{0},".format(p_elem))
                f.write("\n")
