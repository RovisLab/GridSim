...
class DistancesVisualizer(object):
    ...
    def process_parallel_batch(self, gt_batch, pred_batch, image_h, image_w,
                               draw_car, sensor_length, base_path, batch_size, batch_idx):
        car_angle_data = [self.default_car_angle] * len(gt_batch)
        image_list = list()
    ...
    def save_parallel_sensor_data(self, gt, pred, image_h, image_w, batch_size, draw_car,
        ...
        for idx in range(0, size, batch_size):
            self.process_parallel_batch(gt[idx:idx+batch_size],
                                        pred[idx:idx+batch_size],
                                        image_h,
                                        image_w,
                                        draw_car,
                                        sensor_length,
    ...
    def expand_list(self, image_list):
        output = list()
