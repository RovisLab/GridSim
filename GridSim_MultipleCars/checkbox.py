...
class Checkbox:
    def __init__(self, x, y, text, checked=False, text_color=(0, 0, 0)):
        ...
        if pygame.font:
            ...
            self.textDisp = font.render(self.text, 1, self.text_color)
    ...
    def onCheckbox(self, point):
        if point[0] >= self.getX() and point[0] <= (self.getX() + 25 + self.textRect.w) and point[1] >= self.getY() and \
                point[1] <= (self.getY() + 15):
            return True
        else:
            return False
