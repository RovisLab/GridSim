...
class TrafficCar(Car):
    ...
    def trajectory(self, stagePos, screen, object_mask, traffic_car_image, object_car_image, bgWidth, bgHeight,
        ...
        if stagePos[0] >= bgWidth + min_pos_x - screen_w / 2:
            self.data_x = [x + bgWidth for x in self.data_x]
        if stagePos[1] >= bgHeight + min_pos_y - screen_h / 2:
            self.data_y = [x + bgHeight for x in self.data_y]
        if stagePos[0] <= max_pos_x - bgWidth + screen_w / 2:
            self.data_x = [x - bgWidth for x in self.data_x]
        if stagePos[1] <= max_pos_y - bgHeight + screen_h / 2:
            self.data_y = [x - bgHeight for x in self.data_y]
        # ------------------------------------------------------------------------------------
        pos_x = self.data_x[self.index]
        pos_y = self.data_y[self.index]
        if abs(pos_x - stagePos[0]) <= screen_w / 2:
            if abs(pos_y - stagePos[1]) <= screen_h / 2:
                ...
                x = abs(screen_w / 2 - (pos_x - stagePos[0]) - 10)
                y = abs(screen_h / 2 - (pos_y - stagePos[1]) - 10)
