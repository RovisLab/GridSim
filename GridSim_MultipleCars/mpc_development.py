...
class MPCController(object):
    ...
    def control(self, input_data, car_coords):
        ...
        if psi > np.deg2rad(90) and psi < np.deg2rad(270):
            mpc_x = x - car_coords[0]
            mpc_y = y - car_coords[1]
        else:
            mpc_x = car_coords[0]-x
            mpc_y = car_coords[1]-y

        init = (mpc_x, mpc_y, 0, v, cte, epsi, *poly)
